...
export function validateOrRepairFormattedDigest(digest, formatterInput, options, runDir) {
  try {
    ...
    const violations = error?.violations ?? [error instanceof Error ? error.message : String(error)];
    ...
    if (options.modelBackend !== "command") throw error;
    const repairInput = { formatterInput, existingDigest: digest, violations };
