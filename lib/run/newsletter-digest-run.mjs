...
export function validateWorkflowConfig(config) {
  const primarySources = config.sourcePolicy?.primary ?? [];
  const extraCollections = config.sourcePolicy?.extras ?? [];
  const seenKeys = new Set();

  for (const entry of [...primarySources, ...extraCollections]) {
    if (!isNonEmptyString(entry?.key)) {
      throw new Error("Every primary source and extra collection must define a non-empty key.");
    }
    if (seenKeys.has(entry.key)) {
      throw new Error(`Duplicate source or collection key: ${entry.key}`);
    }
    seenKeys.add(entry.key);
  }

  for (const source of primarySources) {
    if (!Array.isArray(source.queryHints) || source.queryHints.filter(isNonEmptyString).length === 0) {
      throw new Error(`Primary source ${source.key} must define at least one queryHint.`);
    }
  }

  for (const extra of extraCollections) {
    const lookbackHours = Number(extra.lookbackHours ?? config.lookbackHours ?? 36);
    if (!Number.isFinite(lookbackHours) || lookbackHours <= 0) {
      throw new Error(`Extra collection ${extra.key} lookbackHours must be a positive number.`);
    }
    if (buildExtraQueries(extra, lookbackHours).length === 0) {
      throw new Error(
        `Extra collection ${extra.key || extra.title || "unknown"} must define gmailLabels or queryHints.`,
      );
    }
  }
}
...
function readSourceArtifact(artifactDir, source, candidate, limits) {
  ...
  const modelContent = compactMarkdownForModel(cleanMarkdown);
  ...
  return {
    ...
    content: trimText(modelContent, maxCleanMarkdownChars),
    maxCleanMarkdownChars,
    originalContentChars: cleanMarkdown.length,
    modelContentChars: modelContent.length,
  };
}
...
export function compactMarkdownForModel(text) {
  return String(text ?? "")
...
function validateGroupShape(group, label, violations) {
  ...
  if (group.kind === "paragraphs") {
    if (!isNonEmptyString(group.content)) {
      violations.push(`${label}.content must be a non-empty string for paragraphs`);
    }
    return;
  }
  if (group.kind === "bullets") {
    if (!Array.isArray(group.content) || group.content.length === 0) {
      violations.push(`${label}.content must be a non-empty array for bullets`);
      return;
    }
    group.content.forEach((item, index) => {
      if (!isNonEmptyString(item)) {
        violations.push(`${label}.content[${index}] must be a non-empty string`);
      }
    });
    return;
  }
  violations.push(`${label}.kind must be paragraphs or bullets`);
}
...
function validateConfiguredGroupPolicy(groups, source, label, violations) {
  const policy = source?.sourceConfig?.formatRules?.validation;
  if (!policy || typeof policy !== "object" || Array.isArray(policy)) return;

  if (Number.isInteger(policy.minGroups) && groups.length < policy.minGroups) {
    violations.push(`${label}.groups must contain at least ${policy.minGroups} groups`);
  }
  if (Number.isInteger(policy.maxGroups) && groups.length > policy.maxGroups) {
    violations.push(`${label}.groups must contain at most ${policy.maxGroups} groups`);
  }
  if (policy.uniqueGroupTitles) {
    const titles = groups.map((group) => String(group?.title ?? "").trim().toLocaleLowerCase());
    if (titles.some((title) => !title) || new Set(titles).size !== titles.length) {
      violations.push(`${label}.groups must use unique non-empty titles`);
    }
  }

  for (const requirement of policy.requiredGroups ?? []) {
    const requiredLabel = `${label}.groups (${requirement.title || "required group"})`;
    const group = groups.find((candidate) => candidate?.title === requirement.title);
    if (!group) {
      violations.push(`${requiredLabel} is required`);
      continue;
    }
    if (requirement.kind && group.kind !== requirement.kind) {
      violations.push(`${requiredLabel}.kind must be ${requirement.kind}`);
    }
    if (group.kind === "bullets" && Array.isArray(group.content)) {
      if (Number.isInteger(requirement.minItems) && group.content.length < requirement.minItems) {
        violations.push(`${requiredLabel}.content must contain at least ${requirement.minItems} items`);
      }
      if (Number.isInteger(requirement.maxItems) && group.content.length > requirement.maxItems) {
        violations.push(`${requiredLabel}.content must contain at most ${requirement.maxItems} items`);
      }
    }
    if (group.kind === "paragraphs" && isNonEmptyString(group.content)) {
      const count = paragraphCount(group.content);
      if (Number.isInteger(requirement.minParagraphs) && count < requirement.minParagraphs) {
        violations.push(`${requiredLabel}.content must contain at least ${requirement.minParagraphs} paragraphs`);
      }
      if (Number.isInteger(requirement.maxParagraphs) && count > requirement.maxParagraphs) {
        violations.push(`${requiredLabel}.content must contain at most ${requirement.maxParagraphs} paragraphs`);
      }
    }
  }
}

function expectedFormatterSections(formatterInput) {
  const expectedSections = [];
  const expectedByKey = new Map();
  for (const source of formatterInput.selectedSources ?? []) {
    const key = source.collectionKey || source.key;
    if (!expectedByKey.has(key)) {
      const expected = { key, sources: [], type: source.type };
      expectedByKey.set(key, expected);
      expectedSections.push(expected);
    }
    expectedByKey.get(key).sources.push(source);
  }
  return expectedSections;
}

export function normalizeFormattedDigestStructure(digest, formatterInput) {
  if (!digest || typeof digest !== "object" || Array.isArray(digest)) return digest;
  const normalized = structuredClone(digest);
  normalized.title = formatterInput.title;
  normalized.date = formatterInput.date;
  normalized.localDate = formatterInput.localDate;
  normalized.inventory = structuredClone(formatterInput.inventory);

  if (!Array.isArray(normalized.sections)) return normalized;
  const expectedSections = expectedFormatterSections(formatterInput);
  const actualByKey = new Map();
  for (const section of normalized.sections) {
    if (!section || typeof section !== "object" || Array.isArray(section)) continue;
    if (!actualByKey.has(section.key)) actualByKey.set(section.key, []);
    actualByKey.get(section.key).push(section);
  }
  const canOrderByKey = expectedSections.every(
    (expected) => actualByKey.get(expected.key)?.length === 1,
  ) && normalized.sections.length === expectedSections.length;
  if (canOrderByKey) {
    normalized.sections = expectedSections.map((expected) => actualByKey.get(expected.key)[0]);
  }

  expectedSections.forEach((expected, sectionIndex) => {
    const section = normalized.sections[sectionIndex];
    const firstSource = expected.sources[0];
    if (!section || typeof section !== "object" || Array.isArray(section) || section.key !== expected.key) return;
    section.title = firstSource.title;
    if (expected.type === "primary") {
      section.type = "primary";
      const allowedUrls = [...sourceUrls(firstSource)];
      if (allowedUrls.length > 0 && !allowedUrls.includes(section.issueLink)) {
        section.issueLink = allowedUrls[0];
      }
      return;
    }

    section.type = firstSource.sectionType || expected.key;
    if (!Array.isArray(section.items) || section.items.length !== expected.sources.length) return;
    section.items.forEach((item, itemIndex) => {
      if (!item || typeof item !== "object" || Array.isArray(item)) return;
      const allowedUrls = [...sourceUrls(expected.sources[itemIndex])];
      if (allowedUrls.length > 0 && !allowedUrls.includes(item.link)) {
        item.link = allowedUrls[0];
      } else if (allowedUrls.length === 0 && isNonEmptyString(item.link)) {
        item.link = "";
      }
    });
  });
...
export function validateFormattedDigest(digest, formatterInput) {
  const violations = [];
  if (!digest || typeof digest !== "object" || Array.isArray(digest)) {
    throw new Error("Formatted digest contract failed: digest must be an object");
  }

  for (const field of ["title", "date", "localDate"]) {
    if (digest[field] !== formatterInput[field]) {
      violations.push(`${field} must exactly match formatter input`);
    }
  }
  if (!isDeepStrictEqual(digest.inventory, formatterInput.inventory)) {
    violations.push("inventory must exactly match formatter input");
  }
  ...
  if (!Array.isArray(digest.sections)) {
    ...
    const actualKeys = digest.sections.map((section) => section?.key);
    const expectedKeys = expectedSections.map((section) => section.key);
    if (!isDeepStrictEqual(actualKeys, expectedKeys)) {
      violations.push(`section keys and order must be ${JSON.stringify(expectedKeys)}`);
    }

    expectedSections.forEach((expected, sectionIndex) => {
      const section = digest.sections[sectionIndex];
      const firstSource = expected.sources[0];
      const label = `sections[${sectionIndex}] (${expected.key})`;
      if (!section || typeof section !== "object" || Array.isArray(section)) {
        violations.push(`${label} must be an object`);
        return;
      }
      if (section.title !== firstSource.title) {
        violations.push(`${label}.title must exactly match formatter input`);
      }

      if (expected.type === "primary") {
        if (section.type !== "primary") violations.push(`${label}.type must be primary`);
        for (const field of ["issueDate", "sender", "issueLink"]) {
          if (!isNonEmptyString(section[field])) violations.push(`${label}.${field} must be a non-empty string`);
        }
        const allowedUrls = sourceUrls(firstSource);
        if (allowedUrls.size === 0) {
          violations.push(`${label}.issueLink cannot be verified because the source has no supplied links`);
        } else if (!allowedUrls.has(section.issueLink)) {
          violations.push(`${label}.issueLink must be a supplied source link`);
        }
        if (!Array.isArray(section.groups) || section.groups.length === 0) {
          violations.push(`${label}.groups must be a non-empty array`);
        } else {
          section.groups.forEach((group, groupIndex) => {
            validateGroupShape(group, `${label}.groups[${groupIndex}]`, violations);
          });
          validateConfiguredGroupPolicy(section.groups, firstSource, label, violations);
        }
        return;
      }

      const expectedSectionType = firstSource.sectionType || expected.key;
      if (section.type !== expectedSectionType) {
        violations.push(`${label}.type must be ${expectedSectionType}`);
      }
      if (!Array.isArray(section.items)) {
        violations.push(`${label}.items must be an array`);
        return;
      }
      if (section.items.length !== expected.sources.length) {
        violations.push(`${label}.items must contain exactly ${expected.sources.length} items`);
      }
      section.items.forEach((item, itemIndex) => {
        const itemLabel = `${label}.items[${itemIndex}]`;
        if (!item || typeof item !== "object" || Array.isArray(item)) {
          violations.push(`${itemLabel} must be an object`);
          return;
        }
        if (!isNonEmptyString(item.title)) violations.push(`${itemLabel}.title must be a non-empty string`);
        if (!isNonEmptyString(item.summary)) violations.push(`${itemLabel}.summary must be a non-empty string`);
        const source = expected.sources[itemIndex];
        const allowedUrls = sourceUrls(source);
        if (allowedUrls.size > 0 && !allowedUrls.has(item.link)) {
          violations.push(`${itemLabel}.link must be a supplied link for the matching source`);
        } else if (allowedUrls.size === 0 && isNonEmptyString(item.link)) {
          violations.push(`${itemLabel}.link must be empty when the source has no supplied links`);
        }
      });
    });
  }
  ...
  if (violations.length > 0) {
    const error = new Error(`Formatted digest contract failed:\n- ${violations.join("\n- ")}`);
    error.violations = violations;
    throw error;
  }

  return {
    expectedSectionKeys: expectedSections.map((section) => section.key),
    extraItemCounts: Object.fromEntries(
      expectedSections
        .filter((section) => section.type === "extra")
        .map((section) => [section.key, section.sources.length]),
    ),
  };
}

export function validateOrRepairFormattedDigest(digest, formatterInput, options, runDir) {
  try {
    const validation = validateFormattedDigest(digest, formatterInput);
    writeJson(modelTempPath(runDir, "format-digest-contract-summary.json"), {
      status: "valid",
      repaired: false,
      ...validation,
    });
    ...
    const initialViolations = error?.violations ?? [error instanceof Error ? error.message : String(error)];
    const invalidPath = modelTempPath(runDir, "format-digest-contract-invalid.json");
    writeJson(invalidPath, digest);
    writeJson(modelTempPath(runDir, "format-digest-contract-error.json"), {
      status: "invalid",
      violations: initialViolations,
      invalidPath,
    });

    const normalized = normalizeFormattedDigestStructure(digest, formatterInput);
    const normalizedPath = modelTempPath(runDir, "format-digest-contract-normalized.json");
    writeJson(normalizedPath, normalized);
    try {
      const validation = validateFormattedDigest(normalized, formatterInput);
      writeJson(modelTempPath(runDir, "format-digest-contract-summary.json"), {
        status: "valid",
        repaired: true,
      ...
      const violations = normalizedError?.violations ?? [
        normalizedError instanceof Error ? normalizedError.message : String(normalizedError),
      ];
      writeJson(modelTempPath(runDir, "format-digest-contract-normalized-error.json"), {
        status: "invalid",
        violations,
        normalizedPath,
      });
      if (options.modelBackend !== "command") throw normalizedError;
      const repairInput = { formatterInput, existingDigest: normalized, violations };
      const repaired = callModelBackend("repair-digest-contract", repairInput, options, runDir);
      try {
        const validation = validateFormattedDigest(repaired, formatterInput);
        writeJson(modelTempPath(runDir, "format-digest-contract-summary.json"), {
          status: "valid",
          repaired: true,
          repairMode: "model",
        ...
        const repairViolations = repairError?.violations ?? [
          repairError instanceof Error ? repairError.message : String(repairError),
        ];
        writeJson(modelTempPath(runDir, "format-digest-contract-repair-error.json"), {
          status: "invalid",
          violations: repairViolations,
          repairOutputPath: modelTempPath(runDir, "repair-digest-contract-output.json"),
        });
        throw repairError;
      }
    }
...
export async function runNewsletterDigest(rawOptions = {}) {
  ...
  if (selectedMessageIds.length === 0) {
    const summary = {
      ...
      selectedMessageIds,
      candidateSummary: resolve(runDir, "candidate-summary.json"),
      reason: "no selected newsletter messages",
    };
    writeJson(resolve(runDir, "usage-summary.json"), summary);
    if (options.outputJson) writeJson(options.outputJson, summary);
    throw new Error("Refusing to send empty newsletter digest: no selected source messages in the lookback window.");
  }
  ...
  const formattedDigest = callModelBackend("format-digest", formatterInput, options, runDir);
  const digest = validateOrRepairFormattedDigest(formattedDigest, formatterInput, options, runDir);
  ...
  const usageSummary = {
    ...
    modelCallsJson: existsSync(resolve(runDir, "model-calls.json"))
      ? resolve(runDir, "model-calls.json")
      : "",
