...
function parseArgs(argv) {
  const options = {
    ...
    formatMode: process.env.NEWSLETTER_DIGEST_FORMAT_MODE || "direct",
    formatTransport: process.env.NEWSLETTER_DIGEST_FORMAT_TRANSPORT || "local",
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (arg) {
      ...
      case "--transport":
        ...
        break;
      case "--format-mode":
        options.formatMode = next ?? "";
        i += 1;
        break;
      case "--format-transport":
        options.formatTransport = next ?? "";
        i += 1;
        break;
      case "--agent":
        options.agent = next ?? "";
  ...
  if (!["direct", "agent"].includes(options.formatMode)) {
    throw new Error("--format-mode must be direct or agent");
  }
  if (!["gateway", "local"].includes(options.formatTransport)) {
    throw new Error("--format-transport must be gateway or local");
  }
...
Options:
  ...
  --format-mode direct|agent  One-call formatter or legacy agent handoff (default: direct)
  --format-transport MODE     Direct formatter transport (default: local)
  --agent ID                  OpenClaw agent for legacy format mode (default: main)
  --agent-timeout-seconds N   Agent timeout for legacy format mode (default: 900)
`);
}
...
function normalizeUsage(usage) {
  ...
  const normalized = {
    input: numericField(usage.input ?? usage.inputTokens ?? usage.prompt_tokens),
    output: numericField(usage.output ?? usage.outputTokens ?? usage.completion_tokens),
    cacheRead: numericField(usage.cacheRead ?? usage.cache_read),
    cacheWrite: numericField(usage.cacheWrite ?? usage.cache_write),
    reasoningTokens: numericField(usage.reasoningTokens ?? usage.reasoning_tokens),
    totalTokens: numericField(usage.totalTokens ?? usage.total_tokens),
    costUsd: numericField(usage.cost?.total ?? usage.costUsd ?? usage.cost),
  };
  return Object.values(normalized).some((value) => value !== null) ? normalized : null;
}

function appendModelCallAudit(options, record) {
  const auditPath = resolve(dirname(options.output), "model-calls.json");
  let calls = [];
  if (existsSync(auditPath)) {
    const existing = readJson(auditPath);
    if (Array.isArray(existing)) calls = existing;
  }
  calls.push(record);
  writeJson(auditPath, calls);
}
...
function formatterSkillInstructions() {
  return readFileSync(formatterSkillPath(), "utf8");
}
...
function buildFormatDigestPrompt(input) {
  return [
    "Format selected newsletter source material into the final digest JSON.",
    "Return exactly one valid JSON object. Do not use markdown outside JSON string values.",
    "Use only the supplied source material. Do not browse, retrieve, render, or send anything.",
    "The workflow policy is already embedded in formatterInput.formatPolicy and each sourceConfig; do not attempt file access.",
    "Preserve title, date, localDate, inventory, source order, and usable supplied links exactly.",
    "Create one primary section per selected primary source.",
    "Create one item section per non-empty extra collection with exactly one item per selected source.",
    "Every extra item must include non-empty title and summary strings. Include publication and link when available.",
    "Follow configured group, validation, and special-case rules. Never emit an empty group.",
    "For groupMode source-subsections, preserve meaningful source subsection headings as separate group titles.",
    "For non-lead roundup groups, use distinct stories from the source's roundup sections rather than details from its lead article.",
    "Before returning, verify every required section and item is present.",
    "",
    "Formatter skill instructions:",
    formatterSkillInstructions(),
    "",
...
function buildRepairDigestContractPrompt(input) {
  return [
    "Repair a newsletter digest that failed the deterministic formatter contract.",
    "Return exactly one valid JSON object. Do not use markdown outside JSON string values.",
    "Use only formatterInput and existingDigest. Do not browse or add facts.",
    "Fix every listed contract violation while preserving valid prose, source order, links, and meaning.",
    "Every extra item must include non-empty title and summary strings.",
    "Do not omit selected sources or invent replacement content.",
    "",
    "Formatter skill instructions:",
    formatterSkillInstructions(),
    "",
...
function buildPrompt(task, input) {
  switch (task) {
    ...
    case "format-digest":
      return buildFormatDigestPrompt(input);
    case "repair-digest-contract":
      return buildRepairDigestContractPrompt(input);
    default:
...
function runOpenClaw(prompt, options) {
  ...
  try {
    ...
    try {
      ...
      // The parser below will report the malformed response. Keep the audit metadata safe and bounded.
    }
    const outputText = response.outputs?.[0]?.text ?? response.text ?? response.output ?? "";
    appendModelCallAudit(options, {
      task: options.task,
      status: "ok",
      ...
      usage: normalizeUsage(response.usage ?? response.outputs?.[0]?.usage),
    });
    return raw;
  } catch (error) {
    appendModelCallAudit(options, {
      ...
      outputBytes: 0,
      usage: null,
      exitStatus: numericField(error?.status),
...
export async function main(argv) {
  ...
  if (options.task === "format-digest" && options.formatMode === "agent") {
  ...
  const directFormatTask = options.task === "format-digest" || options.task === "repair-digest-contract";
  const raw = runOpenClaw(prompt, {
    ...options,
    transport: directFormatTask ? options.formatTransport : options.transport,
  });
...
export const internals = {
  ...
  buildFormatDigestPrompt,
  buildRepairDigestContractPrompt,
