...
function readFormatterContract() {
  ...
  const maxChars = parsePositiveEnvInt("NEWSLETTER_DIGEST_FORMATTER_CONTRACT_CHARS", 8000);
  ...
  if (promptBytes > maxPromptBytes) {
    throw new Error(
      `OpenClaw prompt is ${promptBytes} bytes, above argv-safe limit ${maxPromptBytes}. ` +
        "Reduce maxCleanMarkdownChars, maxSelectedItems, or NEWSLETTER_DIGEST_FORMATTER_CONTRACT_CHARS.",
    );
  }
